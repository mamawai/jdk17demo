...
public class 拾起K个1需要的最少行动次数 {
    public static void main(String[] args) {
        long l = new Solution3086().minimumMoves(new int[]{1, 1, 0, 0, 1, 0, 1, 1, 0, 0, 1}, 4, 0);
...
class Solution3086 {
    public long minimumMoves(int[] nums, int k, int maxChanges) {
        ...
        for (int right = size; right <= n; right++) {
            ...
            // 前面是总长度 index * (i - left) 比如你有三个数 就 index * 3
            // 后面是这几个1的位置的前缀和 做差 sum[i] - sum[left]
            // 三个总长度(即到最后一个0) 1 1 0 0 1 0 是5*3=15
            // 前缀和 0 1 5      15-(5-0) 即为每个1到最后一个0的长度和
            // ----- -> -----
            // ----- ->  ----
            // ----- ->     -
